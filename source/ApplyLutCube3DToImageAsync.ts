import { Expect, OnException, SleepAsync } from "@layer92/core";
import { LutCube3D } from "./LutCube3D";
import { LutCube3DBox } from "./LutCube3DBox";

export async function ApplyLutCube3DToImageAsync(lut:LutCube3D,image:{data:ArrayLike<number>},onProgress?:(completed:number)=>void|Promise<void>,onBadData?:OnException){
    const lutBox = new LutCube3DBox(lut);
    const {data} = image;
    ...
    for(let pixelIndex=0; pixelIndex<imagePixelCount; ++pixelIndex){
        ...
        Expect(r<=1&&g<=1&&b<=1&&r>=0&&g>=0&&b>=0,()=>`Pixel at index ${pixelIndex} is out of range [0, 255].`,onBadData);
